...
class AudioWebSocketHandler {
  ...
  _initializeRoom(roomId) {
    ...
    const room = globalThis.io.sockets.adapter.rooms.get(roomId);
    if (room) {
      const participants = Array.from(room.sockets.values()).map(socketId => {
        const socket = globalThis.io.sockets.sockets.get(socketId);
        return {
...
module.exports = AudioWebSocketHandler;